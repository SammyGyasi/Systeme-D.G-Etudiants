...
class AddStudentForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['course_id'] = forms.ChoiceField(
            label="Course",
            choices=[(course.id, course.course_name) for course in course_choices],
            widget=forms.Select(attrs={"class":"form-control"})
        )

        self.fields['session_year_id'] = forms.ChoiceField(
            label="Session Year",
            choices=[(session.id, f"{session.session_start_year} to {session.session_end_year}") for session in session_choices],
            widget=forms.Select(attrs={"class":"form-control"})
        )
