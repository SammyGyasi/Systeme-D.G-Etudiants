...
from .models import AttendanceReport, Students, Subjects
...
def student_view_attendance(request):
    ...
    for report in attendance_reports:
        subject_attendance = {
            'subject_id': report.attendance_id.subject_id,
            'subject_name': report.attendance_id.subject_id.subject_name,
            'total_presents': AttendanceReport.objects.filter(attendance_id__subject_id=report.attendance_id.subject_id, status=True).count(),
            'total_absences': AttendanceReport.objects.filter(attendance_id__subject_id=report.attendance_id.subject_id, status=False).count()
        }
        subject_attendance_list.append(subject_attendance)
